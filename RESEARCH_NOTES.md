...
what would have to change before any of it counts as real research. These notes are
deliberately candid — they are more useful to a reviewer than a polished pitch.
...
- **The original saved metadata numbers did not all reproduce exactly.** On a current library
  stack the SVM re-runs at ~94.3% (metadata claimed 92.4%) and the GBM at ~99.1% (metadata
  claimed 99.35%). Small shifts are expected across scikit-learn versions; the point is that the
  originally-published numbers should not be quoted to three decimals as if they were fixed.
...
- **Documentation drift.** Earlier documentation contained numbers, clinical claims, hospital
  partnerships, and deployment statistics that the code does not support and that contradicted
  each other across files. These were removed during this cleanup (see the changed-claims list
  in the README / project summary).
